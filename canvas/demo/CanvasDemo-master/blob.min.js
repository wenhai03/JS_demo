var BlobWorld = new (function () {
    ...
    function E() {
        q = j[D(j, {x: u, y: v})];
        q.dragNodeIndex = D(q.nodes, {x: u, y: v})
    }

    function G() {
        var f = {x: u, y: v}, h = j[D(j, f)];
    ...
    function D(f, h) {
        for (var d = 9999, n = 9999, a = -1, o = 0, r = f.length; o < r; o++) {
            n = distanceBetween(f[o].position, {x: h.x, y: h.y});
            if (n < d) {
                d = n;
                a = o
            }
    ...
    function S() {
        ...
        for (n = j.length; h < n; h++) {
            ...
            for (d = 0; d < n; d++) {
                var o = j[d];
                if (o != a)if (distanceBetween({x: a.position.x, y: a.position.y}, {
                        x: o.position.x,
                        y: o.position.y
                    }) < a.radius + o.radius) {
            ...
            for (o = a.nodes.length; d < o; d++) {
                ...
                p = {x: a.position.x, y: a.position.y};
    ...
    //全局变量定义
    //类封装中一个包含全局变量g的全局对象 ，使用方法是： g.variableName
    var g = {
        x: 0,
        y: 0,
        ...
        x: 0,
        y: 1.2
    }, x = {blobA: -1, blobB: -1}, w = 0, C = [{
        fillStyle: "rgba(0,200,250,1.0)", strokeStyle: "#ffffff",
        lineWidth: 10, backgroundColor: "#222222", debug: false
    }, {
        fillStyle: "",
        strokeStyle: "",
        lineWidth: 0,
        ...
        strokeStyle: "rgba(0,0,0,1.0)", lineWidth: 4, backgroundColor: "#000000", debug: false
    }, {
    ...
    this.init = function () {
        if ((t = document.getElementById("world")) && t.getContext) {
            ...
            b({x: g.width * 0.15, y: g.height * Math.random() * 0.2},
                {x: g.width * 0.011, y: 0});
